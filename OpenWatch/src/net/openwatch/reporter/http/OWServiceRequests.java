...
`	 * }
	...
	/**
	 * Fetch server recordingd data, check last_edited, and push update if
	 * necessary
	 * 
	 * @param app_context
	 * @param cb
	 */
	public static void syncRecording(final Context app_context,
		...
		JsonHttpResponseHandler get_handler = new JsonHttpResponseHandler() {
			...
			public void onSuccess(JSONObject response) {
				...
				if (response.has("recording")) {
					try {
						JSONObject recording_json = response
						...
						Log.i(TAG,
								"Sync dates. Remote: "
										...
										+ " local: "
										+ recording.getLastEdited(app_context));
						...
						OWMediaObject media_obj = recording.media_object
								.get(app_context);
						if (recording_json.has(Constants.OW_SERVER_ID)) {
							media_obj.server_id.set(recording_json
						...
						if (recording_json.has(Constants.OW_FIRST_POSTED)) {
							media_obj.first_posted.set(recording_json
						...
						if (recording_json.has(Constants.OW_THUMB_URL)) {
							media_obj.thumbnail_url.set(recording_json
						...
						if (doSave == true) {
							...
							recording.media_object.get(app_context).save(
									app_context);
						...
						if (last_edited_remote.after(last_edited_local)) {
							...
							recording.updateWithJson(app_context,
									recording_json);
						} else if (last_edited_remote.before(last_edited_local)) {
							// copy local to remote
							Log.i(TAG, "local recording data is more recent");
							OWServiceRequests.editRecording(app_context,
									recording, new JsonHttpResponseHandler() {
										@Override
										public void onSuccess(
												JSONObject response) {
											Log.i(TAG,
	...
	public static void getRecordingMeta(Context app_context,
		...
		http_client.get(Constants.OW_API_URL + Constants.OW_RECORDING + "/"
				+ recording_uuid, response_handler);
	}
	...
	public static void editRecording(Context app_context,
		...
		http_client.post(app_context, Constants.OW_API_URL
				+ Constants.OW_RECORDING + "/"
				+ recording.uuid.get().toString(), Utils
				.JSONObjectToStringEntity(recording.toJsonObject(app_context)),
